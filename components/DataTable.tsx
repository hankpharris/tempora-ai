      ...
      <div className="overflow-x-auto">
        ...
        >
          <TableHeader columns={columns}>
            {(column) => (
              <TableColumn key={String(column.key)} className="whitespace-nowrap">
                {column.label}
              </TableColumn>
            )}
          </TableHeader>
            ...
            {(row) => (
              <TableRow key={getRowId(row)}>
                {(columnKey) => {
                  ...
                  const column = columns.find((col) => String(col.key) === columnKeyStr)
                  const value = row[columnKeyStr as keyof T]
                    ...
                    <TableCell
                      ...
                      onClick={() => {
                        if (isEditMode && onCellClick) {
                          onCellClick(row, columnKeyStr, value)
                        }
                      }}
                    >
                      {cellContent}
